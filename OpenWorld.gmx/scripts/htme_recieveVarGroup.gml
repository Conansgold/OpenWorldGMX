...
if (!self.isServer && self.playerhash == "") {
    htme_debugger("htme_recieveVarGroup",htme_debug.INFOR,"Recieved vargroup but we are not continuing: We don't have a playerhash yet.");    
    exit;
}
...
if (!self.isServer && inst_room != room && !inst_stayAlive) {
    htme_debugger("htme_recieveVarGroup",htme_debug.INFOR,"Recieved vargroup but this instance is not in our room (and not stayAlive).");    
    exit;
}
